...
# Image extensions to scan (include .tif/.tiff for microspot; Pillow opens jpg/png/tif/bmp; .ndpi/.pbi may need extra libs)
IMG_EXTS = (".tif", ".tiff", ".jpg", ".jpeg", ".png", ".bmp", ".ndpi", ".pbi")
...
class DeepSpotApp(tk.Tk):
    ...
    def _save_seg_annotation_tiffs(self, image_path, result, seg_path, ann_path):
        """Save full-resolution segmentation and annotation images as .tif (like reference: segmentation1.tif, annotation1.tif)."""
        ...
        try:
            ...
            with Image.open(image_path) as im:
                ...
                for d in result["detections"]:
                    x1, y1, x2, y2 = d["box"]
                    draw.rectangle([x1, y1, x2, y2], outline="#2563eb", width=line_w)
                seg_img.save(seg_path, format="TIFF")
                ann_img = im.copy()
                draw2 = ImageDraw.Draw(ann_img)
                for d in result["detections"]:
                    x1, y1, x2, y2 = d["box"]
                    color = "#10b981" if d["class"] == 0 else "#ef4444"
                    draw2.rectangle([x1, y1, x2, y2], outline=color, width=line_w)
                ann_img.save(ann_path, format="TIFF")
        except Exception as e:
            self._update_status(f"Could not save TIFFs: {e}")
    ...
    def _on_run(self):
        ...
        self._save_seg_annotation_tiffs(
            self.current_image_path, result,
            os.path.join(output_dir, "segmentation1.tif"),
            os.path.join(output_dir, "annotation1.tif"),
        )
        self._export_to_image_folder(output_dir, result, image_names=[os.path.basename(self.current_image_path)], per_image_counts=[result["total_count"]])
        self._auto_export_result(result, image_names=[os.path.basename(self.current_image_path)], update_status=False)
        self._update_status(f"Done. Count={result['total_count']}, Viability={result['viability_pct']}%. segmentation1.tif, annotation1.tif + Result.csv/xlsx/mat in image folder; CSV copy in results/.")

    def _auto_export_result(self, result, image_names=None, per_image_counts=None, update_status=True):
        """Export result to CSV in Var1/Var2 format (Excel-ready) as in the reference screenshot."""
        if image_names is None:
            image_names = []
        if per_image_counts is None:
            per_image_counts = [result["total_count"]] if result else []
        out_dir = get_results_dir()
        os.makedirs(out_dir, exist_ok=True)
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = os.path.join(out_dir, f"Result_{ts}.csv")
        m_v = result["metrics"]["viable"]
        m_a = result["metrics"]["all"]
        viability_decimal = result["viability_pct"] / 100.0
        total_count = result["total_count"]
        c1 = str(per_image_counts[0]) if len(per_image_counts) >= 1 else ""
        c2 = str(per_image_counts[1]) if len(per_image_counts) >= 2 else ""
        rows = [
            ("Count in image 1", c1),
            ("Count in image 2", c2),
            ("Total count", str(total_count)),
            ("Viability", f"{viability_decimal:.9f}"),
            ("Avg diameter (live) (um)", f"{m_v['avg_diameter_um']:.6f}"),
            ("Avg diameter (total) (um)", f"{m_a['avg_diameter_um']:.6f}"),
            ("Avg area (live) (um^2)", f"{m_v['avg_area_um2']:.6f}"),
            ("Avg area (total) (um^2)", f"{m_a['avg_area_um2']:.6f}"),
            ("Avg perimeter (live) (um)", f"{m_v['avg_perimeter_um']:.6f}"),
            ("Avg perimeter (total) (um)", f"{m_a['avg_perimeter_um']:.6f}"),
            ("Avg perimeter over square root of area (live)", f"{m_v['avg_perimeter_sqrt_area']:.6f}"),
            ("Avg perimeter over square root of area (total)", f"{m_a['avg_perimeter_sqrt_area']:.6f}"),
            ("Avg circularity (live)", f"{m_v['avg_circularity']:.6f}"),
            ("Avg circularity (total)", f"{m_a['avg_circularity']:.6f}"),
            ("Avg aspect ratio (live)", f"{m_v['avg_aspect_ratio']:.6f}"),
            ("Avg aspect ratio (total)", f"{m_a['avg_aspect_ratio']:.6f}"),
        ]
        with open(path, "w", encoding="utf-8") as f:
            f.write("Var1,Var2\n")
            for var1, var2 in rows:
                f.write(f'"{var1}","{var2}"\n')
        if update_status:
            self._update_status(f"Done. Result exported to {path}")

    def _export_to_image_folder(self, folder, result, image_names=None, per_image_counts=None, filename_prefix="Result"):
        """Write Result.xlsx and Result.mat (and Result.csv) into folder, like reference layout."""
        if image_names is None:
            image_names = []
        if per_image_counts is None:
            per_image_counts = [result["total_count"]] if result else []
        ...
        try:
            ...
            for var1, var2 in rows:
                ws.append([var1, var2])
            wb.save(os.path.join(folder, f"{filename_prefix}.xlsx"))
        except ImportError:
            pass
        try:
            import scipy.io
            mat_dict = {
                ...
                "viability_pct": result["viability_pct"],
                "count_image_1": c1,
                "count_image_2": c2,
                "avg_diameter_um_live": m_v["avg_diameter_um"],
                "avg_diameter_um_total": m_a["avg_diameter_um"],
                "avg_area_um2_live": m_v["avg_area_um2"],
                "avg_area_um2_total": m_a["avg_area_um2"],
                "avg_perimeter_um_live": m_v["avg_perimeter_um"],
                "avg_perimeter_um_total": m_a["avg_perimeter_um"],
                "avg_circularity_live": m_v["avg_circularity"],
                "avg_circularity_total": m_a["avg_circularity"],
                "avg_aspect_ratio_live": m_v["avg_aspect_ratio"],
                "avg_aspect_ratio_total": m_a["avg_aspect_ratio"],
            }
            scipy.io.savemat(os.path.join(folder, f"{filename_prefix}.mat"), mat_dict)
        except ImportError:
            pass
    ...
    def _on_batch_run(self):
        ...
        if output_dir and os.path.isdir(output_dir):
            for i, path in enumerate(self.image_files):
                res = self.all_detection_results.get(path)
                if res:
                    n = i + 1
                    self._save_seg_annotation_tiffs(
                        path, res,
                        os.path.join(output_dir, f"segmentation{n}.tif"),
                        os.path.join(output_dir, f"annotation{n}.tif"),
                    )
            if self.all_detection_results:
                results_list = [self.all_detection_results[p] for p in self.image_files]
                total_sum = sum(r["total_count"] for r in results_list)
                viable_sum = sum(r["viable_count"] for r in results_list)
                overall_viability = (100.0 * viable_sum / total_sum) if total_sum else 0
                last_res = results_list[-1]
                per_image_counts = [r["total_count"] for r in results_list]
                agg = {**last_res, "total_count": total_sum, "viable_count": viable_sum, "viability_pct": round(overall_viability, 1)}
                self._export_to_image_folder(
                    output_dir, agg,
                    image_names=[os.path.basename(p) for p in self.image_files],
                    per_image_counts=per_image_counts,
                )
        self._update_status(f"Batch complete. {len(self.image_files)} images logged. Seg/annotation TIFFs + Result.xlsx/.mat in folder.")
        # Auto-export batch summary to app results dir (CSV with timestamp)
        if self.all_detection_results:
            results_list = [self.all_detection_results[p] for p in self.image_files]
            total_sum = sum(r["total_count"] for r in results_list)
            viable_sum = sum(r["viable_count"] for r in results_list)
            ...
            per_image_counts = [r["total_count"] for r in results_list]
            agg = {**last_res, "total_count": total_sum, "viable_count": viable_sum, "viability_pct": round(overall_viability, 1)}
            self._auto_export_result(agg, image_names=[os.path.basename(p) for p in self.image_files], per_image_counts=per_image_counts)
